...
class DLPSlicerGUI(QWidget):
    ...
    def __init__(self, dlp_controller=None, parent=None):
        ...
        self.__init_opengl_widget__()
        self.__init_options_widget__()
        self.main_layout.addWidget(self.__opengl_widget)
        self.main_layout.addWidget(self.__options_widget)
        self.setLayout(self.main_layout)

    def __init_opengl_widget__(self):
        self.__opengl_widget = DLPSlicer(parent=self, dlp_controller=self.dlp_controller)
        self.__opengl_widget.update_physical_size.connect(self.update_size_label)
        self.__opengl_widget.update_fps.connect(self.update_fps_label)
        self.__opengl_widget.update_slice_counts.connect(self.update_slices_label)
    ...
    def __init_options_widget__(self):
        ...
        self.__options_layout.addWidget(self.__slicer_widget, 1, 2)
    ...
    def __init_geometry_widget__(self):
        ...
        self.rotate_x_slider.valueChanged.connect(self.__opengl_widget.set_x_rotation)
        self.rotate_x_spin.valueChanged.connect(self.rotate_x_slider.setValue)
        self.rotate_x_spin.valueChanged.connect(self.__opengl_widget.set_x_rotation)
        ...
        self.rotate_y_slider.valueChanged.connect(self.__opengl_widget.set_y_rotation)
        self.rotate_y_spin.valueChanged.connect(self.rotate_y_slider.setValue)
        self.rotate_y_spin.valueChanged.connect(self.__opengl_widget.set_y_rotation)
        ...
        self.rotate_z_slider.valueChanged.connect(self.__opengl_widget.set_z_rotation)
        self.rotate_z_spin.valueChanged.connect(self.rotate_z_slider.setValue)
        self.rotate_z_spin.valueChanged.connect(self.__opengl_widget.set_z_rotation)
        ...
        self.translate_x_slider.valueChanged.connect(self.__opengl_widget.set_x_pos)
        self.translate_x_spin.valueChanged.connect(self.translate_x_slider.setValue)
        self.translate_x_spin.valueChanged.connect(self.__opengl_widget.set_x_pos)
        ...
        self.translate_z_slider.valueChanged.connect(self.__opengl_widget.set_z_pos)
        self.translate_z_spin.valueChanged.connect(self.translate_z_slider.setValue)
        self.translate_z_spin.valueChanged.connect(self.__opengl_widget.set_z_pos)
    ...
    def __init_slicer_widget__(self):
        self.__slicer_widget = QGroupBox("Slicer Options", self)

        thickness_label = QLabel("Layer Thickness", self.__slicer_widget)
        thickness_edit = QDoubleSpinBox(self.__slicer_widget)
        ...
        thickness_edit.valueChanged.connect(self.__opengl_widget.set_slice_thickness)

        pixel_size_label = QLabel("Projector Pixel Size", self.__slicer_widget)
        pixel_size_edit = QDoubleSpinBox(self.__slicer_widget)
        ...
        pixel_size_edit.valueChanged.connect(self.__opengl_widget.set_pixel_size)

        projector_resolution_label = QLabel("Projector Resolution", self.__slicer_widget)
        projector_resolution_edit_x = QSpinBox(self.__slicer_widget)
        ...
        projector_resolution_edit_x.valueChanged.connect(self.__opengl_widget.set_projector_width)
        projector_resolution_edit_y = QSpinBox(self.__slicer_widget)
        ...
        projector_resolution_edit_y.valueChanged.connect(self.__opengl_widget.set_projector_height)

        samples_per_pixel_label = QLabel("Samples per Pixel", self.__slicer_widget)
        samples_per_pixel_edit = QSpinBox(self.__slicer_widget)
        ...
        samples_per_pixel_edit.valueChanged.connect(self.__opengl_widget.set_samples_per_pixel)
        ...
        slice_interrupt_button.clicked.connect(self.__opengl_widget.interrupt_slicing)
        ...
        slice_layout = QGridLayout(self.__slicer_widget)
        ...
        self.__slicer_widget.setLayout(slice_layout)

    @Slot(float)
    def set_scaling(self, value):
        if self.uniform_scaling.isChecked():
            self.__opengl_widget.set_x_scale(value)
            self.__opengl_widget.set_y_scale(value)
            self.__opengl_widget.set_z_scale(value)
            self.scale_x_spin.setValue(value)
            self.scale_y_spin.setValue(value)
            self.scale_z_spin.setValue(value)
        else:
            self.__opengl_widget.set_x_scale(self.scale_x_spin.value())
            self.__opengl_widget.set_y_scale(self.scale_y_spin.value())
            self.__opengl_widget.set_z_scale(self.scale_z_spin.value())
    ...
    def load_geometry(self):
        ...
        for file_name in file_names[0]:
            ...
            if file_extension.lower() == 'obj':
                if self.__opengl_widget.load_obj(file_name, swapyz):
                    self.geometry_list.addItem(self.__opengl_widget.geometry_name_list[self.__opengl_widget.geometries_loaded-1])
                    self.geometry_list.setCurrentIndex(self.geometry_list.count() - 1)
            elif file_extension.lower() == 'stl':
                if self.__opengl_widget.load_stl(file_name, swapyz):
                    self.geometry_list.addItem(self.__opengl_widget.geometry_name_list[self.__opengl_widget.geometries_loaded - 1])
                    self.geometry_list.setCurrentIndex(self.geometry_list.count() - 1)
    ...
    def remove_geometry(self):
        self.__opengl_widget.remove_geometry()
    ...
    def start_slicing_process(self):
        ...
        if len(directory_name) > 0:
            self.__opengl_widget.start_slicing(directory=directory_name)
    ...
    def update_unit_of_measure(self, index):
        self.__opengl_widget.set_unit_of_measurement(self.list_of_measures_coefficients[index])

    @Slot(int)
    def update_geometry_transformations(self, index):
        if index >= 0:
            self.__opengl_widget.current_geometry_idx = index
            x_rot = self.__opengl_widget.get_x_rot()
            y_rot = self.__opengl_widget.get_y_rot()
            z_rot = self.__opengl_widget.get_z_rot()
            x_scale = self.__opengl_widget.get_x_scale()
            y_scale = self.__opengl_widget.get_y_scale()
            z_scale = self.__opengl_widget.get_z_scale()
            x_pos = self.__opengl_widget.get_x_pos()
            z_pos = self.__opengl_widget.get_z_pos()
            unit_measurement = self.__opengl_widget.get_unit_of_measurement()
