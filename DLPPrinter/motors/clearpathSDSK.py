...
class ClearpathSDSK(QObject):
    ...
    @Slot()
    def move_building_plate(self, distance_mm, feed_rate, relative_move=True):
        if self.ser.isOpen():
            try:
                if relative_move:
